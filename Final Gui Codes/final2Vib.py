...
def send_command(command):
    try:
        ser.write(f"{command}\n".encode('utf-8'))  # Ensure newline for Arduino parsing
...
def update_serial_connection():
    ...
    try:
        ...
        ser.reset_input_buffer()
        print(f'Connection on {selected_com_port} successful')
...
group_3e = ctk.CTkFrame(group_3)   # Start machine
...
high_feedrate_button = ctk.CTkButton(group_3c, text="HIGH FEED (1000)", command=lambda: send_command('highFeedRate'))
low_feedrate_button = ctk.CTkButton(group_3d, text="LOW FEED (5000)", command=lambda: send_command('slowFeedRate'))
start_machine_button = ctk.CTkButton(group_3e, text="START MACHINE", command=start_machine)
...
start_machine_button.pack(padx=10, pady=10)
...
group_3e.pack(padx=5, pady=5, fill=ctk.BOTH, expand=True)
